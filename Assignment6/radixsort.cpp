...
void reassemble (cl_command_queue &queue,
        cl_context &context,
    ...
    int len)
{
  ...
  CHK_ERR(err);
  err = clSetKernelArg(reassemble_kern, 1, sizeof(cl_mem), &zeros);
  CHK_ERR(err);
  err = clSetKernelArg(reassemble_kern, 2, sizeof(cl_mem), &ones);
  CHK_ERR(err);
  err = clSetKernelArg(reassemble_kern, 3, sizeof(cl_mem), &out);
  CHK_ERR(err);
