		   ...
		   int n)
{
  ...
  __local int buffer[128];
  
  for (int offset = 1; offset < dim; offset *= 2) {
    if (tid >= offset)
      buffer[tid - offset] = buf[tid - offset];

    barrier(CLK_LOCAL_MEM_FENCE);
    
    if (tid >= offset)
      buf[tid] = buffer[tid - offset] + buf[tid];
...
__kernel void reassemble(__global int *in, __global int *zeros, __global int *ones,
                         __global int *out, __local int *buf, int k, int n) 
{
